...
On 2026-09-13 the procedural Run of ATLAS and AETHER, and later the same day of SERAPH, RONIN, SCORPIO, TITAN and VANGUARD, was replaced by the Quaternius CC0 `Jog_Fwd_Loop` retargeted locally (`scripts/update_library_run.py`); no provider calls or credits. The earlier run refinement below was also local: `motion_library.py` now authors foot contact, rolling, recovery and body counter-rotation using character-specific timing. Both Blender sources and GLBs include it.
