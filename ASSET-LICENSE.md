...
- Project software and documentation use the [MIT license](LICENSE).
- Quaternius source animations already use CC0; their original notices remain in [assets/animations/quaternius](assets/animations/quaternius/README.md).
- Google's pose-detection model and MediaPipe runtime in `public/tracking/` are third-party tracking components, not mecha assets. Their upstream terms remain applicable.
- Third-party sample photography used in browser test screenshots is not part of this dedication.
- Dependency code and externally loaded fonts keep their own licenses. See [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).

CC0 does not grant rights belonging to unrelated third parties, trademark or patent rights, or endorsement. The assets are supplied without warranties, as provided in CC0.
