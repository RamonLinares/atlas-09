...
- [Universal Animation Library 1](https://quaternius.itch.io/universal-animation-library): Walk_Loop, Jog_Fwd_Loop, Sprint_Loop, Punch_Jab, Punch_Cross, Death01, Hit_Chest, Hit_Head, Roll.
...
Every character’s Run is `Jog_Fwd_Loop` via `scripts/update_library_run.py -- <character>` (or `all`): the loop is closed onto its first pose over the last 15% of the cycle, pelvis bounce and foot lift are damped (40% / 55% for ATLAS, 45% / 60% for AETHER, 38–42% / 52–58% for the others) because vertical travel scales with the mecha’s leg length, the cycle is slowed per mass (1.0× AETHER to 1.25× TITAN), and the legs are re-solved on the boot geometry. Carried equipment (SERAPH’s cannon and wings, RONIN’s katana and skirt, SCORPIO’s claws and tail, VANGUARD’s rifle and skirt) is posed after the retarget by the script’s per-character adjuster. `Sprint_Loop` is preserved in the subset as the faster alternative (`--with-sprint` bakes it for comparison).
