...
ATLAS’s and AETHER’s Run are `Jog_Fwd_Loop` via `scripts/update_library_run.py -- <character>`: the loop is closed onto its first pose over the last 15% of the cycle, pelvis bounce and foot lift are damped (40% / 55% for ATLAS, 55% / 70% for AETHER) because vertical travel scales with the mecha’s leg length, and the legs are re-solved on the boot geometry. `Sprint_Loop` is preserved in the subset as the faster alternative (`--with-sprint` bakes it for comparison).
