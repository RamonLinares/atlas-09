"""Inspect a preserved Tripo retopology before assigning rigid parts.

Usage: blender -b --python scripts/inspect_classic.py -- <character-id> <height-m>
Writes normalized front/side/back renders, coordinate plots and slice data to output/<character-id>/.
"""
import bpy,math,json,sys,numpy as np
from pathlib import Path
from mathutils import Matrix,Vector
argv=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else []
CHAR=argv[0] if argv else 'titan-06';HEIGHT=float(argv[1]) if len(argv)>1 else 19.0
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'output'/CHAR;OUT.mkdir(parents=True,exist_ok=True)
sources=list((ROOT/'assets'/CHAR/'retopo').glob('*model*.glb')) or list((ROOT/'assets'/CHAR/'source').glob('*model*.glb'))
...
pts=[Matrix.Rotation(-math.pi/2,4,'Z')@(o.matrix_world@v.co) for v in o.data.vertices];low=min(p.z for p in pts);scale=HEIGHT/(max(p.z for p in pts)-low)
...
report={'source':str(sources[0].relative_to(ROOT)),'scale':scale,'triangles':sum(len(f.vertices)-2 for f in o.data.polygons),'vertices':len(p),'bounds':[[float(p[:,i].min()),float(p[:,i].max())] for i in range(3)],'slices':{}}
for z in np.arange(.25,HEIGHT+.25,.25):
 q=p[abs(p[:,2]-z)<.125]
 if len(q):
  entry={'count':int(len(q)),'x':[round(float(q[:,0].min()),2),round(float(q[:,0].max()),2)],'y':[round(float(q[:,1].min()),2),round(float(q[:,1].max()),2)]}
  # x-extent histogram helps find gaps between torso and arms / between legs.
  hist,edges=np.histogram(q[:,0],bins=np.arange(math.floor(p[:,0].min()),math.ceil(p[:,0].max())+.5,.5))
  entry['x_occupancy']=''.join('#' if h else '.' for h in hist);entry['x_bins_start']=float(edges[0])
  report['slices'][f'{z:.2f}']=entry
...
bpy.ops.object.camera_add(location=(0,-45,HEIGHT/2));c=bpy.context.object;c.data.type='ORTHO';c.data.ortho_scale=HEIGHT+4;aim(c)
...
for name,pos in [('front',(0,-45,HEIGHT/2)),('side',(45,0,HEIGHT/2)),('back',(0,45,HEIGHT/2))]:
