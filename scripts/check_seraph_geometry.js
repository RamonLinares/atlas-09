async page => {
 await page.waitForFunction(()=>window.atlas?.stats.character==='seraph-03');
 const reports=[];
 for(const width of [1440,390]){
  await page.setViewportSize({width,height:width===390?844:1000});
  for(const [label,name] of [['IDLE','Sentinel'],['RUN','Run'],['KNEEL & FIRE','KneelFire'],['BACKFLIP','Backflip'],['DEPLOY WINGS','WingDeploy'],['FLIGHT','Flight']]){
 ...
 const firing=await page.evaluate(()=>{
  const states=[];
  for(const time of [2.42,2.8,3.06,3.7]){atlas.mixer.setTime(time);atlas.model.updateMatrixWorld(true);states.push({time,...atlas.motionFX.update('KneelFire',time)})}
  const m=atlas.motionFX.muzzle,fore=atlas.model.getObjectByName('forearmR'),hand=atlas.model.getObjectByName('handR');
