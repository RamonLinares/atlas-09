...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'output/ronin-04';OUT.mkdir(parents=True,exist_ok=True)
sources=list((ROOT/'assets/ronin-04/retopo').glob('*model_url.glb')) or list((ROOT/'assets/ronin-04/source').glob('*model_url.glb'))
...
pts=[Matrix.Rotation(-math.pi/2,4,'Z')@(o.matrix_world@v.co) for v in o.data.vertices];low=min(p.z for p in pts);scale=16/(max(p.z for p in pts)-low)
...
bpy.ops.object.camera_add(location=(0,-45,10));c=bpy.context.object;c.data.type='ORTHO';c.data.ortho_scale=20;aim(c)
