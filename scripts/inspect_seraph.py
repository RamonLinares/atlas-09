"""Inspect the winged source in three views before assigning any rigid parts."""
import bpy,bmesh,math,json,numpy as np
from pathlib import Path
from mathutils import Matrix,Vector
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'output/seraph-03';OUT.mkdir(parents=True,exist_ok=True)
sources=list((ROOT/'assets/seraph-03/retopo').glob('*model_url.glb')) or list((ROOT/'assets/seraph-03/source').glob('*model_url.glb'))
...
pts=[Matrix.Rotation(-math.pi/2,4,'Z')@(o.matrix_world@v.co) for v in o.data.vertices];low=min(p.z for p in pts);scale=20/(max(p.z for p in pts)-low)
...
# A numerical side silhouette makes wing/arm depth separation measurable.
...
point=Vector((0,0,10))
def aim(o):o.rotation_euler=(point-o.location).to_track_quat('-Z','Y').to_euler()
...
bpy.ops.object.camera_add(location=(0,-45,10));c=bpy.context.object;c.data.type='ORTHO';c.data.ortho_scale=24;aim(c)
