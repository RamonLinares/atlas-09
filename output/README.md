...
Character-specific reports live in their named folders. Shared motion reports are under `motion/`; browser captures are under `playwright/`. `webcam-rig-validation.json` covers the first five skeletons, including upper-body fallback. `webcam-upper-body-validation.json` records detection from a test image cropped above the hips.
