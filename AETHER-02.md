...
The generated mesh had yaw, roll and a lateral offset in its rest pose. `build_aether.py` applies a coarse -35-degree correction followed by a measured rigid alignment of the helmet’s bilateral plane to the torso’s x=0 plane. The head bone is vertical, and AETHER’s idle and run clips have no side-to-side head sway. The Blender source and downloadable GLB both include the correction. Rigid weights and UVs remain valid.
